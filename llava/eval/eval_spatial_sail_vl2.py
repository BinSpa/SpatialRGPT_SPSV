# Evaluation script for SAIL-VL2-8B models on SpatialRGPT-Bench
# Based on official SAIL-VL2 usage: https://huggingface.co/BytedanceDouyinContent/SAIL-VL2-8B
...
def preprocess_image(image_path, image_size=IMAGE_SIZE):
    ...
    pixel_values = torch.from_numpy(np.array(image)).permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W] uint8
    pixel_values = pixel_values.to(torch.bfloat16) / 255.0  # normalize to [0, 1]
    return pixel_values


def eval_model(args):
    """Evaluate SAIL-VL2 model on SpatialRGPT-Bench."""

    model, tokenizer = load_model_and_tokenizer(args.model_path, args.cache_dir)

    # Set system message on the model's conversation template
    ...
    # Generation config — SAIL-VL2's chat() expects a dict, not GenerationConfig
    generation_config = {
    ...
    for batch_idx in tqdm(range(total_batches), desc="Evaluating batches"):
        ...
        for line in batch_questions:
            ...
            # Load and preprocess image from som_images folder
            image_path = os.path.join(args.image_folder, 'som_images', image_id + '.png')
            pixel_values = preprocess_image(image_path).to(model.device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate SAIL-VL2 models on SpatialRGPT-Bench"
    )

    parser.add_argument("--model-path", type=str, default="BytedanceDouyinContent/SAIL-VL2-8B",
